...
interface StaffPortalProps {
  onRejectJob: (job: Job, staffName: string, reason: string) => void;
}

const StaffPortal: React.FC<StaffPortalProps> = ({ onRejectJob }) => {
  ...
  const handleAvailabilityUpdate = (updatedAvailability: Staff['availability']) => {
    ...
    if (staffIndex !== -1) {
      ...
      setCurrentStaff({ ...staffMembers[staffIndex] });
    }
  };
  ...
  const submitRejection = () => {
    ...
    if (jobToReject) {
      onRejectJob(jobToReject, currentStaff.name, reason.trim());
    }
